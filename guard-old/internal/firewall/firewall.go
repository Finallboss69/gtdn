...
const (
	...
	powerShellTimeout   = 8 * time.Second  // Timeout más corto
...
type Manager struct {
	...
	blockSec     int
	workerSem    chan struct{} // Semáforo para limitar concurrencia de PowerShell
...
// executeBlock ejecuta el comando PowerShell para bloquear una IP
func (m *Manager) executeBlock(ip string) error {
	ruleName := rulePrefix + strings.ReplaceAll(ip, ".", "-")
	script := fmt.Sprintf(
		`New-NetFirewallRule -DisplayName "%s" -Direction Inbound -Action Block -RemoteAddress "%s" -ErrorAction SilentlyContinue`,
		ruleName, ip,
	)

	ctx, cancel := context.WithTimeout(m.ctx, powerShellTimeout)
	defer cancel()

	cmd := exec.CommandContext(ctx, "powershell.exe", "-NoProfile", "-NonInteractive", "-Command", script)
	// Descargar stdout y stderr para evitar buffers
	cmd.Stdout = io.Discard
	cmd.Stderr = io.Discard

	err := cmd.Run()
	if err != nil {
		// Si el contexto fue cancelado, no es un error real
		if ctx.Err() == context.DeadlineExceeded {
			return fmt.Errorf("timeout ejecutando PowerShell para IP %s", ip)
		}
...
// executeUnblock ejecuta el comando PowerShell para desbloquear una IP
